...
void sTimer::SetMoveTiming(void) {
    ...
    if (data[TIME] >= 0) {
        ...
        isSmartManagement = true;

        if (data[MOVES_TO_GO] == 1)
            data[TIME] -= Min(1000, data[TIME] / 10);
...
int sTimer::FinishIteration(void) {
    ...
    if (isRecapture && isSmartManagement && !isRootMoveChanging) {
        return (GetElapsedTime() >= ((moveTime * percentage) / 100) / 2);
    }
...
int sTimer::TimeHasElapsed(void) {
    return (GetElapsedTime() >= (moveTime * percentage) / 100);
}
...
{
    if (isSmartManagement) {
        isRootMoveChanging = true;
        percentage += 4;
        if (percentage > 125) percentage = 125;
    }
}
